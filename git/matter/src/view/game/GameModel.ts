...
class GameModel{    
    ...
    public randStar():void{
        ...
        let lvl:number = Number(cfgLevel.arrPass[this.lvlIndex<cfgLevel.arrPass.length?this.lvlIndex:1]);
        let arrConfig:Array<Cfg_Pass>= DataConfig.ins.getPasss(lvl);
        let len:number = arrConfig.length;
        this.arrStarNeed=[];
        for(let i=0;i<len;i++){
            let id:number=i;
            let cfg:Cfg_Pass = arrConfig[i];
